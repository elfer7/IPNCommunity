    ...
    protected void doGet(HttpServletRequest peticion, HttpServletResponse respuesta) throws ServletException, IOException
    {
        ...
        if(sesionActiva)
        {
            try
            {
                ...
                XPathExpression<Element> elementos = fabrica.compile("/preguntas/pregunta[@categoria=\""+peticion.getParameter("c")+"\"]", Filters.element()); // Crashea si no utiliza los binarios de Jaxen
...
"               <div class=\"row\">");
                p.println(new Sidebar("e", ruta, peticion.getParameter("c")).writeHTML());
                if(peticion.getParameter("c") != null)
                    p.println(new Mainbar("cat", peticion.getParameter("c"), resultados, ruta).writeHTML());
                p.println("</div></div>");
